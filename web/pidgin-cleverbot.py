...
#    if flags == 2 and username == "myusername@hotmail.com":
    if flags == 2:
        if conversation not in conversations.keys():
            ...
            purple.PurpleConvImSend(purple.PurpleConvIm(conversation), u'Hi! I\'m Amy! P̴l is not around right now...')
        else:
#            question = print(message)
            response = conversations[conversation].Ask(message)
            print 'Asked: "%s" Response: "%s"' % (message, response)
